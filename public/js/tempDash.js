...
let a = setAllDataForADate(date)

function setAllDataForADate() {

  let a = getBiometricData(date);
  let b = getAllEmpInfo();
  let c = getAttendanceRecord();

  Promise.all([a, b, c]).then(function(values) {
    let getBiometricData = values[0];
    let getAllEmpInfo = values[1];
    let getAttendanceRecord = values[2];

    $('#totalEmployeesInDB').html(getAllEmpInfo.totalEmployeesInDB);
    allEmpInfo = getAllEmpInfo.employeeInfo;

    document.getElementById("camera_count").innerHTML = getAttendanceRecord.length;
    // res = removeNoise(res);
    generateAttendanceTable(getAttendanceRecord, date)
    totalEmployeesPresentInFR = getAttendanceRecord.length
    workerCountShiftC = getAttendanceRecord.length

    if() {
      $('#biometricCount').html("A- " + getBiometricData.value.numWorkerA + " C- " + getBiometricData.value.numWorkerC)
      ...
      dirIndirChartData['series'][1]['data'] = [getBiometricData.value.numWorkerA, getBiometricData.value.numWorkerC]
      Highcharts.chart('directIndirectBar', dirIndirChartData);
    } else {
      $('#biometricCount').html(getBiometricData.countPresent)
      $('#biometricPlant').html(plant)
      dirIndirChartData['xAxis']['categories'] = [
        'Day Shift'
      ]
      dirIndirChartData['series'][0]['data'] = [totalEmployeesPresentInFR]
      dirIndirChartData['series'][1]['data'] = [getBiometricData.countPresent]
      Highcharts.chart('directIndirectBar', dirIndirChartData);
    }
