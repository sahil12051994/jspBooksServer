  ...
  return new Promise((resolve,reject) => {
    ...
    getFrameData(startTime,endTime,camID).then(function(value){
      ...
      createGif(frameList, fName).then(function(link){
        ...
        console.log("unable to resolve createGif", err);
      });
    }).catch(err => {
      console.log("unable to resolve getFrameData", err);
    });
  })
}
...
function getFrameData(startTime,endTime,camID) {
  return new Promise((resolve, reject) => {
    ...
    query.then(function(doc) {
      ...
      console.log("unable to resolve Processed query in getFrameData", err);
    });
  });
}
...
function createGif(pics, fName) {
  return new Promise(function(resolve, reject) {
    let gif = new GifEncoder(800, 600);
